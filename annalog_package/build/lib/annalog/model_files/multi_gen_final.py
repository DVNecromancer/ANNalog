...
def beam_search_decode(
    ...
    with torch.no_grad():
        ...
        for _ in range(tokens_to_generate):
            ...
            # Handle 3x beam width for filter_invalid
            if filter_invalid:
            ...
            for indexes, prob in zip(trg_indexes, trg_probs):
                if indexes[-1] == trg_field.vocab.stoi[trg_field.eos_token]:
                    if filter_invalid:
                        # Validate finished sequences if filter_invalid is ON
                        ...
                        # Record all finished sequences if filter_invalid is OFF
                        final_sequences.append((indexes, prob))
                        finished_sequences.append(indexes)
...
def sampling_decoder(
    model, src_tensor, src_mask, trg_field, max_len, temperature, num_sequences, device, use_masking=False, seed=42, prefix_length=0, filter_invalid=False
):
    """
    ...
    with torch.no_grad():
        ...
        while len(all_sequences) < num_sequences:
            ...
            for _ in range(remaining_sequences):
                ...
                # Validate the full SMILES if filter_invalid is enabled
                if filter_invalid:
